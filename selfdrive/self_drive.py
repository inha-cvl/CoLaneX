...
class SelfDrive:
    def __init__(self, map):
        ...
        # self.mpc = MPC(**config['control']['mpc'], vehicle_length=config['common']['vehicle_length'], vehicle_width=config['common']['vehicle_width'], wheelbase=config['common']['wheelbase'],steer_max=config['common']['steer_max'], dsteer_max=config['common']['dsteer_max'], max_velocity =config['planning']['velocity_profile']['max_velocity'])
        self.p_steer_cmd = 0
    ...
    def execute(self, mode, vehicle_state, path, lidar_object):
        ...
        # acc_cmd, steer_cmd = self.mpc.get_mpc_result(vehicle_state)
